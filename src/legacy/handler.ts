...
async function handleUserController(ctx: LegacyContext, action: string): Promise<NextResponse> {
  switch (action.toLowerCase()) {
    ...
    case "register": {
      ...
      `;
      if (existing.length > 0) {
        return jsonError("账号已存在");
      }
      await sql`
        INSERT INTO "user"(mail, password, create_time, register_ip)
        VALUES (${username}, ${md5(password)}, ${nowDateTimeString()}, ${getRealIp(ctx.request)})
      `;
    ...
    case "forgetpass": {
      ...
      const userRows = await sql<{ id: number }[]>`
        SELECT id FROM "user" WHERE mail = ${username} LIMIT 1
      `;
    ...
    case "get": {
      ...
      >`
        SELECT id, mail, manager, nickname, avatar, qq_open_id, active::text AS active
        FROM "user"
        WHERE id = ${authUser.user_id}
        LIMIT 1
      `;
  ...
  switch (action.toLowerCase()) {
    ...
    case "installcard": {
      ...
      try {
        ...
        return jsonError("Next.js 版本暂不支持在线下载插件，请手动部署插件目录");
      } catch (error) {
    ...
    case "build": {
      ...
      return jsonError("Next.js 版本暂不支持在线打包浏览器扩展");
    }
    case "folders": {
      await getAdmin(ctx);
      const { authCode, authService } = initAuthConfig(ctx);
